...
#Change working dir relative to data folder


#Load all 3 decades for this version
ninety1_LBS <- read.csv("data/census/Scotland/1991_Scotland_LBS_CoBPostcodeSectors_allPeople_notMF_countyNamesAdded.csv")
twoThousand1 <- read.csv("data/census/Scotland/2001_Scots_CoB_univariatetable_countryNamesAdded.csv")
#twoThEleven <- read.csv("2011/Scotland/OutputArea_bulk_CoB_formatted/QS203SC_formatted.csv", as.is = T)
#use comma-less version
twoThEleven <- read.csv("data/census/Scotland/OutputArea_bulk_CoB_formatted/QS203SC_formatted_commasRemoved.csv")

#version with all columns to get the indexing actually correct
namesNoHeadinz <- read.csv("data/census/1971to2011_CountryOfBirthCategories_Scotland_noRegionHeadings_allCols.csv")
...
lapply(1:length(savens),function(x) write.csv(results[[x]], 
                                 paste0("data/census/VariableCoding/CountryOfBirth_threeCensusRecodes_to91LBS/",savens[x],".csv"),
                                 row.names = F))